  ...
  return (
    <div className="p-2 border-bottom border-3 border-dark">
      ...
      {props.players.map(player => <div key={player.id}><div className="d-inline-block mb-1">
        ...
        <span className="text-bg-warning p-2">{player.points} Point{player.points !== 1 ? 's' : ''} {player.added > 0 ? <span className="fw-bold">(+ {player.added} Point{player.added > 1 ? 's' : ''})</span> : ''}</span>
